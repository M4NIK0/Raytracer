...
int main()
{
    for (int i = 0; i < 100; i++)
    {
        ...
        auto obj1 = std::make_shared<raytracer::Sphere>(raytracer::Point3D(0.5, -101, -4), 100,
                                                        raytracer::Color(1, 1, 1));
        auto obj2 = std::make_shared<raytracer::Sphere>(raytracer::Point3D(/*-1.5*/0 + (double)i / 50, 0.5, -5), 1,
                                                        raytracer::Color(1, 0, 1));
        auto obj3 = std::make_shared<raytracer::Sphere>(raytracer::Point3D(0.5, 1.7, -4), 0.1,
                                                        raytracer::Color(1, 1, 1));
        auto obj4 = std::make_shared<raytracer::Sphere>(raytracer::Point3D(0.2, -0.3, -3), 0.7,
                                                        raytracer::Color(1, 1, 1));
//    auto obj5 = std::make_shared<raytracer::Sphere>(raytracer::Point3D(0.8, 0, -109), 100, raytracer::Color(0, 1, 1));
        auto obj5 = std::make_shared<raytracer::Plane>(raytracer::Point3D(0.8, 0, -109), 100,
                                                       raytracer::Color(0, 1, 1));
        ...
        renderer.renderer.addLight(
                std::make_shared<raytracer::PointLight>(raytracer::Color(255, 0, 0), raytracer::Point3D(-50, 200, -25),
                                                        40000));
        renderer.renderer.addLight(
                std::make_shared<raytracer::PointLight>(raytracer::Color(0, 255, 0), raytracer::Point3D(0, 200, -25),
                                                        40000));
        renderer.renderer.addLight(
                std::make_shared<raytracer::PointLight>(raytracer::Color(0, 0, 255), raytracer::Point3D(50, 200, -25),
                                                        40000));

        renderer.renderImageCLI();

        // Create PPM Output
        raytracer::PPMOutput output("./output" + std::to_string(i) + ".ppm", WIDTH, HEIGHT);
