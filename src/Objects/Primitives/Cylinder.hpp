...
namespace raytracer
{
    ...
    {
    public:
        Cylinder(raytracer::Point3D pos, double radius, double height, Color surfaceReflexion) : _radius(radius), _height(height) , _position(std::move(pos)), _surfaceAbsorbtion(surfaceReflexion), _volumeAbsorbtion(surfaceReflexion), _emissionColor({0, 0, 0}) { _surfaceAbsorbtion.normalize(); _volumeAbsorbtion.normalize(); };
        ~Cylinder();
